...
def CAN_RCV_LOOP():
    while 1:
        ...
        if can_id == 0x208:
            ...
            print("Speed: %06d Torque: %06d Data: 0x%02x 0x%02x" % (Chassis_1_Speed.output, Chassis_1_Torque.output,int(chs1_out/256), int(chs1_out-int(chs1_out/256))))
            can_pkt = struct.pack(fmt, 0x208,8,bytes([0,0,0,0,0,0,int(chs1_out/256),int(chs1_out-int(chs1_out/256))]))
            #sock.send(can_pkt)
